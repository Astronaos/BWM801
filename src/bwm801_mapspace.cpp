...
{
	unsigned int uiList;
	if (m_uiHex_Background_List == (unsigned int)(-1))
	{
		uiList = glGenLists(1);
	...
	{
		if (m_uiHex_Side_List[uiSide] == (unsigned int)(-1))
		{
			uiList = glGenLists(1);
			glNewList(uiList,GL_COMPILE);
	...
	if (m_uiHex_Outline_List == (unsigned int)(-1))
	{
		uiList = glGenLists(1);
		glNewList(uiList,GL_COMPILE);
			Draw_Hex_Outline();
	...
	if (m_uiQuad_Background_List == (unsigned int)(-1))
	{
		uiList = glGenLists(1);
		glNewList(uiList,GL_COMPILE);
			Draw_Filled_Hex();
	...
	{
		if (m_uiQuad_Side_List[uiSide] == (unsigned int)(-1))
		{
			uiList = glGenLists(1);
			glNewList(uiList,GL_COMPILE);
				Draw_Hex_Side(uiSide);
	...
	if (m_uiQuad_Outline_List == (unsigned int)(-1))
	{
		uiList = glGenLists(1);
		glNewList(uiList,GL_COMPILE);
			Draw_Hex_Outline();
...
void mapspace::Draw_Hex_Side(unsigned int i_uiSide) const
{
	...
	if (m_uiHex_Side_List[i_uiSide] == (unsigned int)(-1))
	{
		glBegin(GL_LINES);
...
void mapspace::Draw_Quad_Side(unsigned int i_uiSide) const
{
	...
	if (m_uiQuad_Side_List[i_uiSide] == (unsigned int)(-1))
	{
		glBegin(GL_LINES);
